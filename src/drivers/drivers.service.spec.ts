...
describe('DriversService', () => {
  ...
  beforeEach(async () => {
    ...
    const validatorMock = {
      ...
      assertUserExists: jest
        .fn()
        .mockResolvedValue({ id: USER_ID, role: 'DRIVER', isActive: true }),
  ...
  describe('create()', () => {
    ...
    it('validates userId when linking to a user account', async () => {
      ...
      expect(validator.assertUserExists).toHaveBeenCalledWith(
        TENANT_ID,
        USER_ID,
        'User',
      );
      expect(validator.assertUserNotAlreadyDriver).toHaveBeenCalledWith(
        USER_ID,
      );
    });
    ...
    it('propagates ConflictException when license is already taken', async () => {
      validator.assertLicenseUnique.mockRejectedValue(
        new Error('License taken'),
      );
      await expect(service.create(TENANT_ID, dto)).rejects.toThrow(
        'License taken',
      );
  ...
  describe('update()', () => {
    it('updates driver name successfully', async () => {
      ...
      const result = await service.update(TENANT_ID, DRIVER_ID, {
        name: 'New Name',
      });
    ...
    it('throws NotFoundException when driver not found', async () => {
      (prisma.driver.findFirst as jest.Mock).mockResolvedValue(null);
      await expect(
        service.update(TENANT_ID, 'missing', { name: 'X' }),
    ...
    it('validates new phone uniqueness when phone changes', async () => {
      ...
      expect(validator.assertPhoneUnique).toHaveBeenCalledWith(
        '+1-555-NEW',
        DRIVER_ID,
      );
    });
    ...
    it('throws BadRequestException when no updatable fields provided', async () => {
      (prisma.driver.findFirst as jest.Mock).mockResolvedValue(makeDriver());
      await expect(service.update(TENANT_ID, DRIVER_ID, {})).rejects.toThrow(
        BadRequestException,
      );
    });
  });
